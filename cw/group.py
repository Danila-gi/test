class Students_record:
    def __init__(self, name, suspicion):
        self.name = name
...
class Group_heap:
    ...
    def add_record(self, record: Students_record):
        if self.heap_size == 0:
    ...
    def remove_record(self, student: Students_record):
        i = 0
        for i in range(0, self.heap_size):
            if student.name == self.heap[i].name:
                break
